...
enum Rank: Int {
    ...
    func greaterThan(otherRank: Rank) -> Bool {
...
king.greaterThan(ace)
ace.greaterThan(king)
...
var ranktwo = Rank(rawValue: 2)
ranktwo?.simpleDescription()
...
struct Card {
    ...
    func createDeck() -> [Card] {
        var ranks = {
            ...
            let numRanks = 13
            for i in 1...numRanks {
                ranks.append(Rank(rawValue: i)!)
            }
            return ranks
        }()
        
        func cardsForSuit(suit: Suit) -> [Card]{
            var cards = [Card]()
            for rank in ranks {
        ...
        var cards = [Card]()
        cards += cardsForSuit(Suit.Clubs)
        cards += cardsForSuit(Suit.Diamonds)
        cards += cardsForSuit(Suit.Hearts)
        cards += cardsForSuit(Suit.Spades)
        return cards
    }
}
...
let cont = ServerResponse.Continue("More cheese incoming")
...
switch cont {
    ...
    case let .Continue(message):
        print("Continue... \(message)")
}

let cardSuit = Suit.Clubs
switch cardSuit {
    default:
        print(cardSuit.color())
}
